...
const Navigation: React.FC<NavigationProps> = ({
  ...
  return (
    <nav
      className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? "bg-white/90 backdrop-blur-md border-b border-stone-100 py-3" : "bg-transparent py-5"}`}
    >
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="absolute top-full left-0 w-full bg-white border-t border-stone-100 shadow-xl p-6 flex flex-col gap-4 animate-in slide-in-from-top-5">
          {["Khám phá", "Tính năng", "Cộng đồng", "Blog"].map((item) => (
            <a
              key={item}
              href="#"
